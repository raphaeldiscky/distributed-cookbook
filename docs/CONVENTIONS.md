...
K8s-mode recipes (`envoy-gateway`, `cilium-gateway`) bind the host
port via kind cluster `extraPortMappings` (one port per recipe,
forwarding to the gateway data plane's NodePort/Service). Each recipe
runs in its own kind cluster, so the ports don't conflict at runtime —
but the registry above still tracks them globally to prevent two
recipes ever picking the same port.
...
**Universal services** (`services/<name>/`) follow the same shape but
use the **service name** as subsystem:

```
Namespace: "cookbook"
Subsystem: "<service-name underscored>"   (e.g. user_service, catalog_service)
Name:      "<metric_name>"
```

So `cookbook_user_service_requests_total`,
`cookbook_catalog_service_request_duration_seconds`, etc. Service
names and recipe names live in disjoint sets so collision is
impossible.
...
flashsale it's `oversell_total`. (Performance-comparison recipes like
`envoy-gateway` / `cilium-gateway` may pick a latency-distribution panel as their hero
instead — note the deviation in RECIPE.md.)
...
## 9. Kubernetes-mode recipes (deployments/k8s, helm, Tilt)

Most recipes run as a host Go binary against docker-compose infra (see
§ 7). Recipes whose lesson is K8s-native (gateway-API controllers,
service meshes, operators) use **Mode B**: kind + Helm + Tilt.

```
recipes/<name>/                 # K8s-mode recipe — self-contained
├── RECIPE.md
├── Tiltfile                    # tilt up -f recipes/<name>/Tiltfile
├── kind-cluster.yaml           # kind config (extraPortMappings, CNI choice)
├── scripts/
│   └── kind-up.sh              # bootstrap (cluster + recipe-specific CNI/CRDs)
├── grafana/<name>.json
└── loadtest/*.js
...
│   │   ├── servicemonitor.yaml
│   │   └── kustomization.yaml
│   └── recipes/<name>/         # recipe-specific wiring (one Gateway, one HTTPRoute)
│       ├── kustomization.yaml  # composes services/* bases + recipe pieces
│       ├── namespace.yaml
│       ├── gateway.yaml
│       └── httproute.yaml
└── helm/<chart>/                # release.yaml + values.yaml per chart
    ├── cilium/
    ├── envoy-gateway/
...
- **One kind cluster per recipe**, named `cookbook-<recipe>` (e.g.
  `kind-cookbook-envoy-gateway`). Each recipe owns its
  `kind-cluster.yaml` and `scripts/kind-up.sh` so its bootstrap is
  fully self-contained. `task kind_up RECIPE=<name>` creates it.
- **One Tiltfile per recipe** at `recipes/<name>/Tiltfile`. Run via
  `task tilt_up RECIPE=<name>`. ArgoCD is for prod, not here.
- **One gateway/ingress per K8s recipe** — production-realistic
  topology. Comparison between implementations happens by running each
  recipe in turn (different cluster, different host port), not by
  cramming two implementations into one cluster.
- **Monitoring lives inside the kind cluster** via
  `kube-prometheus-stack` (helm). The compose-based LGTM stack is for
  Mode A (compose) recipes only.
- **Grafana dashboards** are delivered via ConfigMaps with the label
  `grafana_dashboard=1`; the kube-prom Grafana sidecar auto-loads them.
  Source JSON lives at `recipes/<name>/grafana/<name>.json`; the
  ConfigMap wrapper is generated by Tilt at apply time so there's
  only one source of truth.
- **Services are reused via kustomize base references**: a recipe's
  kustomization includes `../../services/<name>` rather than copying
  workload manifests. Recipes own only the recipe-specific wiring.
- **Helm chart pins** live in `deployments/helm/<chart>/release.yaml`
  (read by Tiltfile via `read_yaml()` and by `kind-up.sh` via `yq`).
  See `.claude/rules/pinned-versions.md`.
- **Port mappings**: each recipe claims one host port from § 1's
  registry. Avoid 3000 (compose Grafana) and 9090 (compose Prometheus);
  the in-kind monitoring uses 3001 and 9091.

## 10. Go package layout inside a recipe
