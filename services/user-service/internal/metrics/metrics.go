// Package metrics defines the Prometheus metrics user-service emits.
//
// Service metric namespace convention (see docs/CONVENTIONS.md § 2):
//
//	namespace = cookbook
//	subsystem = <service-name underscored> (here: user_service)
//
// This keeps service metrics distinct from recipe metrics (which use
// the recipe name as subsystem) on a shared Prometheus.
package metrics
...
func New(reg prometheus.Registerer) *Metrics {
	m := &Metrics{
		Requests: prometheus.NewCounterVec(prometheus.CounterOpts{
			...
			Name:      "requests_total",
			Help:      "User-service HTTP requests labeled by route and status.",
		}, []string{"route", "status"}),
		Latency: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			Namespace: "cookbook",
			Subsystem: "user_service",
			Name:      "request_duration_seconds",
			Help:      "User-service handler latency, per route.",
